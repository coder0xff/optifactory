...
def parse_material_list(text):
    ...
    for item in text.split(','):
        ...
        if not item:
            continue
        if ':' not in item:
            raise ValueError(f"Invalid format: '{item}'. Expected 'Material:Rate'")
        
        material, rate_str = item.split(':', 1)
        ...
        try:
            ...
            raise ValueError(f"Invalid rate '{rate_str}' for {material}. Must be a number.") from exc
...
def parse_mine_list(text):
    ...
    for item in text.split(','):
        ...
        try:
            ...
            raise ValueError(f"Invalid purity '{purity_str}'. Must be IMPURE, NORMAL, or PURE.") from exc
...
Examples:
  ...
  %(prog)s --outputs "Iron Plate:100" --mines "Iron Ore:PURE, Iron Ore:NORMAL"
    ...
    parser.add_argument(
        '--output-file', '-f',
        help='Write graphviz output to file instead of stdout'
    )
